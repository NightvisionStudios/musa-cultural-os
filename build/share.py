...
<section class="keypin">
  ...
  <div class="kbody">
    ...
    <div class="ktxt">Save this and run it behind any card — it carries the whole key: score, heat, all three flags and the tier ladder. <b>High Score + Low Heat = a FIND</b>: the work already holds up, the room hasn't arrived, and you're getting put onto it before it gets loud. That gap is the whole board.</div>
    <div class="kacts">
...
/* BENCHMARK IS OUT OF THE CAPTION (2026-08-01), for the same reason it came
   off the card image itself a day earlier: a caption travels with the card,
   away from the archive, and "Benchmark in THE 50 — <surname>" stripped of
   that framing reads as a flat comparison rather than a placement. The
   benchmark still runs on the permalink, where THE 50 is one tap away and can
   carry it. Do not add it back here. */
function caption(d){{
  const L=[];
  L.push(d.name);

...
def _score_rail(d, entry, x, y, boxw, score_px=96, chip_fs=21, heat=True, bw=160):
    ...
    for f_ in [z for z in (entry.get("flags") or []) if "BLADE" not in str(z).upper()]:
        ...
        col = GREEN if "FIND" in lbl else (SAND if "EARLY" in lbl else AMBER)
        bor = "#2f3a28" if "FIND" in lbl else ("#3a352a" if "EARLY" in lbl else "#3a2f1c")
...
KEY_FLAGS = [
    ...
    ("ROOM'S EARLY", SAND,  "#3a352a", "the room is ahead of what's landed so far."),
]
